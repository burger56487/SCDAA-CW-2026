...
class LQR_Solver:
    ...
    def _get_S_and_integral(self, t_batch):
        if self.S_interp is None or self.integral_interp is None:
            raise RuntimeError("Riccati solution not available. Call solve_riccati() first.")
        ...
        S_tensor = torch.tensor(
            S_flat_np.T, dtype=torch.float32, device=t_batch.device
        ).reshape(-1, 2, 2)

        integral_tensor = torch.tensor(
            integral_np, dtype=torch.float32, device=t_batch.device
        ).reshape(-1, 1)
    ...
    def value_function(self, t_batch, x_batch):
        ...
        x_S_x = torch.bmm(torch.bmm(x_batch, S_t), x_batch.transpose(1, 2))
...
def run_mc_lqr(lqr_solver, x0, T, N_steps, N_samples):
    ...
    time_grid = np.linspace(0.0, T, N_steps, endpoint=False)
    S_flat_np = lqr_solver.S_interp(time_grid)
    S_seq = torch.tensor(
        S_flat_np.T, dtype=torch.float32, device=device
    ).reshape(N_steps, 2, 2)
...
def plot_convergence(lqr_solver, x0, T):
    ...
    anchor_idx_1 = min(2, len(N_steps_list) - 1)
    ref_line_1 = [errors_time[anchor_idx_1] * (N_steps_list[anchor_idx_1] / n) for n in N_steps_list]
    ...
    plt.subplot(1, 2, 2)
    plt.loglog(N_samples_list, errors_samples, "s-", label="MC Error")

    anchor_idx_2 = min(2, len(N_samples_list) - 1)
    ref_line_2 = [
        errors_samples[anchor_idx_2] * np.sqrt(N_samples_list[anchor_idx_2] / n)
    ...
    print("\nTable for MC sample-size experiment:")
    print("MC samples | MC value | Absolute error")
    for n_samp, v_mc, err in zip(N_samples_list, mc_values_samples, errors_samples):
        print(f"{n_samp:10d} | {v_mc:8.4f} | {err:.6f}")
